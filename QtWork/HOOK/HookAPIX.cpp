...
BOOL CXHookAPI::InlineHookAddress( LPVOID lpAddress,FILTERROUTINE pRoutine )
{
	...
	bRet = VirtualProtectEx(hHandle, m_pTempCode, sizeof(m_pTempCode), PAGE_EXECUTE_READWRITE, &dwOldProtect);
	if (FALSE == bRet)
	{
		CloseHandle(hHandle);
		return FALSE;
	...
	bRet = VirtualProtectEx(hHandle, lpCode, nLen, PAGE_EXECUTE_READWRITE, &dwOldProtect);
	if (FALSE == bRet)
	{
		CloseHandle(hHandle);
		return FALSE;
	...
	bRet = VirtualProtectEx(hHandle, lpCode, nLen, dwOldProtect, &dwOldProtect);
	if (FALSE == bRet)
	{
		CloseHandle(hHandle);
		return FALSE;
